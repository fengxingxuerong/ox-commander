...
describe("FileJournal", () => {
  ...
  it("构建/测试产物不算任务变更：批内跑 npm run build 不该让整批越权", () => {
    // 这些目录名会出现在**任何**跑过构建的目标项目里。把它们当源码变更计入
    // 越权清单的后果是：任务全在 zone 内、批却永远验收不过，而默认档
    // （`revert-batch`）还会去回滚一个**由构建产生**的文件。
    const root = scratch("journal-generated");
    write(root, "src/a.js", "a");
    write(root, "dist/stale.js", "old bundle");
    const journal = new FileJournal();
    const token = journal.begin(root, ["src"]);
    write(root, "src/b.js", "b"); // 任务自己的产出
    write(root, "dist/new.js", "bundle"); // 批内构建
    write(root, "coverage/lcov.info", "TN:\n"); // 批内测试
    write(root, "__pycache__/m.pyc", "pyc"); // 批内 import

    expect(journal.changed(token).map((c) => c.path)).toEqual(["src/b.js"]);
    // 被跳过的目录里**排着同级文件**时，同级仍要被看见（不能退化成整树不遍历）
    write(root, "d-sibling.js", "s");
    expect(journal.changed(token).map((c) => c.path).sort()).toEqual(["d-sibling.js", "src/b.js"]);
  });

  it("名为 dist / build 的**文件**不享有目录跳过", () => {
    const root = scratch("journal-generated-file");
    write(root, "dist", "a shell script named dist");
    const journal = new FileJournal();
    const token = journal.begin(root, ["src"]);
    write(root, "dist", "rewritten by the task");
    expect(journal.changed(token).map((c) => c.op + ":" + c.path)).toEqual(["modify:dist"]);
  });
...
describe("BatchGuard arbitration", () => {
  ...
  it("批内构建产物既不判越权、也不被回滚删除（默认档）", async () => {
    // 这是验收判据的一半：任务在 zone 内写完，另外跑了项目自带的构建 ——
    // 整批必须照常通过，且构建产物不能被还原掉。
    const root = scratch("guard-build-output");
    const backupRoot = scratch("guard-build-output-backups");
    write(root, "src/a.js", "a");
    const events: string[] = [];
    const guard = new BatchGuard({
      snapshots: new SnapshotStore({ backupRoot }),
    ...
    const scope = await guard.begin("r-build", root, ["src"]);
    write(root, "src/a.js", "a-updated");
    write(root, "dist/bundle.js", "generated by npm run build");

    const verdict = await guard.settle(scope, [{ taskId: "t1", ok: true, logDigest: "ok", events: [] }]);
    expect(verdict.conflicts).toEqual([]);
    expect(verdict.outcomes[0]!.ok).toBe(true);
    expect(read(root, "dist/bundle.js")).toBe("generated by npm run build");
    expect(read(root, "src/a.js")).toBe("a-updated");
...
describe("FileJournal · 遍历与比对的跳过语义", () => {
  it("[127] 未变化的文件被跳过后，排在后面的修改仍要被发现", () => {
    // 第 127 行 `if (before.size === now.size && before.mtimeMs === now.mtimeMs) continue;`。
  ...
  it("[164] 递归完子目录后，父目录后面的同级文件仍要被遍历", () => {
    // 第 164 行是 `walk(abs)` 之后那句 `continue;`。改成 `break` 后，
  ...
  it("[168] 既非文件也非目录的条目（junction）被跳过后，后面的同级文件仍要被遍历", () => {
    // 第 168 行 `if (!entry.isFile()) continue;`。改成 `break` 后，

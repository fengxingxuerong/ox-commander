...
**剩余 4 处**（第七批后，全部集中在 `orchestrator.ts`）。

### 16.13 第七批：两个大集成层（2026-09-23）

| 文件 | site 口径 | 说明 |
| --- | --- | --- |
| `electron/agents/manifest-schema.ts` | 42/55 → **55/55** | 13 处一批全杀 |
| `electron/engine/orchestrator.ts` | 16/25 → **21/25** | 5 处收口，**剩 4 处** |

**清单校验（manifest-schema）** 的共同形态：既有用例只喂过
「类型正确、取值非法」的输入（负数、空白串），从没喂过**类型就不对**的
（数字塞进字符串字段、字符串塞进数组字段）—— 于是多条件 `||` 里靠后的
条件从未被求值，改坏后要么抛 **TypeError**（校验的全部价值就是那句
"哪个字段错了"），要么把非法值放行。

两个特别值得记的：
- `bool()` 入口 `v === undefined || v === null` 改 `!==` 后**恒真** →
  **所有布尔字段被吞掉**（`enabled: false` 变缺省）。
- `Object.values("K=V")` 会把字符串拆成字符数组，**每个字符都是 string**
  → 非对象 envTemplate 反而"校验通过"。**构造错值时要专门试字符串。**

**调度主链路（orchestrator）**：存活项都在修复轮循环体里，改坏后
**不抛错、只是行为悄悄变了**；断言落点只能是可观测外部事实
（回调收到了什么、日志写了什么）。

**⚠️ 成本教训**：`@408` 第一版把触发条件构造反了，跑完 **123s** 的全量审计
才发现没杀掉。改进闭环 —— **手工注入单个变异 + 只跑该目标的测试文件**
（5s）→ 先钉死单条断言，**最后才跑全量审计做验收**。
